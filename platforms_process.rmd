...
In this analysis, I download and analyze Republican and Democratic presidential platforms going back to 1968. I use visualizations and text analysis to determine how and whether they vary by party and year.
...
repubs <-
  paste0(
    "https://www.presidency.ucsb.edu/documents/republican-party-platform-",
    seq(1968, 2020, by = 4)
  )
dems <-
  paste0(
...
urls[c(9, 10, 11, 12, 13, 14)] <-
  map_chr(c(9, 10, 11, 12, 13, 14),
          ~ str_replace(urls[.x], "([a-z-]+)-(\\d{4})", "\\2-\\1"))
urls[14] <-
  str_replace(urls[14],
              "\\d.*",
...
I parse the raw text line by line and create tibbles by sentence and word. Since in 2020 the Republicans did not adopt a new platform, I drop the Republicans for that year. I decide that this was less disruptive than double-counting the 2016 platform or dropping the 2020 Democrats as well.
```{r, cache = TRUE, message = FALSE}
by_word <-
  raw[-14] %>%  map(~ str_extract_all(.x, regex(
    "^[:alpha:].*?[[:punct:]-[)]]$", multiline = TRUE
  ))) %>%
  map(unlist) %>%
  ...
  map(~ filter(.x, !str_detect(word, "[\\d,.]+"))) %>%
  map(mutate, sen_number = as.numeric(as_factor(sentence))) %>% 
...
```{r}
year_scale <-
  partial(scale_x_continuous,
          breaks = seq(1968, 2020, by = 4),
          ... = )
...
overall %>% count(party, year) %>%
  ...
  labs(
    title = "Word Count by Year and Party",
    color = "Party",
    x = "Year",
    y = "Count"
  )
...
## tf-idf
Term frequency-inverse document frequency measures a term's commonness in one document relative to other documents. It assumes that words that are unusually common in certain documents convey significant meaning.
In this corpus, the terms with the highest tf-idf were mostly the names of one of the two candidates, either from the same party or the opposing one. This makes sense; most of those names would only be topical in one election cycle. The "cent" in the Democratic platform of 1976 comes from rendering "percent" as two words.
...
There is surprising variation in word frequency rank distribution, with the longer platforms having more variety.
```{r}
overall %>% mutate(across(c(party, year), as.factor)) %>%
  ...
  labs(
    title = "Word Frequency Rank by Year",
    x = "Rank",
    y = "Word Frequency (log10)",
    color = "Year"
  )
```
...
Average sentiment in most party-years was mildly positive, with no year above 1. The outlier is 1972, with a far more positive Republican than Democratic platform. That is not what we'd expect from a comparison of second-term Nixon and George McGovern, which might indicate a problem with the approach.
```{r}
library(lubridate)
...
sent_means %>%
  ...
  ggplot(aes(
    x = fct_reorder(factor(year), rev(sort(year))),
    y = diff,
    fill = factor(year)
  )) +
  geom_col() +
...
## By Passage
Which passages had the most extreme sentiment scores? I filter for the highest and lowest AFINN score means. The most negative passage of all castigates Bill Clinton for vetoing the partial-birth abortion ban; the most positive seems to be a 1984 Republican section extolling Reagan's policy successes. From these results, the coding scheme seems sound.
```{r}
sent_sentiments %>%
  mutate(sen_number = (sen_number %/% agg) + 1) %>%
  group_by(sen_number, party, year) %>%
  summarize(
    sen_number = unique(sen_number),
    sentence = paste(sentence, collapse = " "),
    `AFINN Mean` = mean(`AFINN Mean`)
  ) %>%
  ungroup() %>%
  group_by(year) %>%
...
# By Incumbency
What fraction of words in each platform is positive? In incumbent years, it seems the party defending the White House adopts a more positive tone than the challenging party.
```{r}
word_sentiments %>% mutate(value = ifelse(value <= 0, 0, 1)) %>%
  group_by(year, party) %>%
  summarize(`Proportion Positive` = mean(value)) %>%
  mutate(mins = c(pmin(
    `Proportion Positive`[party == "R"], `Proportion Positive`[party == "D"]
  ), NA),
  maxes = c(pmax(
    `Proportion Positive`[party == "R"], `Proportion Positive`[party == "D"]
  ), NA)) %>%
  ...
  scale_color_manual(values = platform_scale) +
  geom_errorbar(
    aes(xend = year, ymin = mins, ymax = maxes),
    col = "navajowhite",
    width = 3,
    size = .75
  ) +
  ...
  geom_line() +
  ylim(c(0, .8)) +
  labs(title = "Proportion Positive by Party and Year",
       x = "Year",
       y = "Proprotion of Words Positive",
       col = "Party")
```
...
by_defender %>% group_by(defender) %>%
  ...
  labs(title = "Proportion Positive by Party Defending",
       x = "Defending Party",
       y = "Proportion Positive",
       fill = "Defending Party") +
  theme(legend.position = "none")
```

Mean positivity for defending parties was about 4% higher than for challengers. A difference-of-means test confirms the difference is significant, though not very substantial.
```{r}

by_defender %>% mutate(
  challenging = if_else((defender == "Republican" &
                           party == "D") | defender == "Democrat" & party == "R",
                        "Challenging",
                        "Defending"
  ) %>% as_factor()
) %>%
...
grid %>% left_join(tab) %>%
  ...
  labs(title = "Significance Results of Pairwise ANOVA")
```

# Topic Analysis

Topic modeling attempts to extract the underlying structure of the corpus. It searches for words that commonly co-occur and develops topics that contain differing probabilities of each word. It produces two statistics: $\beta$, a word's probability of appearing by topic, and $\gamma$, a document's probability of containing a topic.

## By Party
Does a two-topic model capture variation by party? I count words by party and try a model with two topics. The second topic appears more Republican, though "Democrats" appears in both. Overall, the difference isn't striking enough to be persuasive.
```{r}
library(topicmodels)
library(tidytext)
...
Differentiation is now clear; it's not hard to guess which year corresponds to the topic with the words "reagan" and "soviet."
```{r, cache = TRUE}
...
topics_y %>% tidy() %>%
  ...
  ggplot(aes(
    x = fct_reorder(factor(topic), beta_mean),
    y = beta_mean,
    fill = factor(topic)
  )) +
  geom_col() +
...
Topic 6 had the highest average beta among the top 20 words, indicating its most frequent words were relatively more common than those of other topics. The year must be 1976 or 1980, from the reference to Carter.
```{r}
topics_y %>% tidy() %>%
  filter(topic == 6) %>%
  arrange(-beta) %>%
...
It looks like the year-topic correspondence did the worst for the 1980s but performed well in other years. Perhaps there was less variation that decade because Republicans controlled the presidency for almost all of it?
```{r}
...
topics_y %>% tidy(matrix = "gamma") %>%
  ggplot(aes(
    x = factor(document),
    y = factor(topic, ordered = TRUE, levels = ord),
    fill = gamma
  )) +
  geom_tile(color = "black") +
  scale_fill_gradient(low = "white", high = "red") +
  labs(title = "Gamma by Topic and Year",
       x = "Year",
       y = "Topic",
       fill = "Gamma") +
...
topic_words %>% ggplot(aes(
  x = fct_reorder(factor(term), beta, sum),
  y = beta,
  fill = factor(topic)
)) +
  (topic_words %>% mutate(term = fct_reorder(factor(term), beta, sum)) %>%
     ...
     map( ~ geom_col(
       ...
       aes(
         x = factor(term),
         y = beta,
         fill = fct_reorder(factor(topic), beta)
       )
     ))) +
  coord_flip() +
  scale_x_discrete(labels = levels(fct_reorder(
    factor(topic_words$term), topic_words$beta, sum
  ))) +
  coord_flip() +
  theme(axis.ticks.y = element_blank(),
        axis.text.y = element_text(size = 8)) +
  labs(title = "Combined Betas of Top Topic Words",
       x = "Term",
       y = "Beta",
       fill = "Topic") +
...
topics_y %>% tidy(matrix = "beta") %>%
  ...
  bind_rows() %>%
  ggplot(aes(
    x = fct_reorder(factor(term), beta, max),
    y = beta,
    fill = factor(topic)
  )) +
  ...
  labs(x = "Term",
       y = "Beta",
       fill = "Topic",
       title = "Beta Distribution for Top Topic Words")
```

# Conclusion

Party platforms varied strongly by year, and a one-topic-per-year model differentiated them clearly. Democratic and Republican platforms overall were fairly similar. The most commonly used words tended to be generic ("American", "Support", etc.), but many platforms contained words common only in that year (often the names of candidates). Average platform sentiment was mildly positive overall, significantly more so for the incumbent's party. Many further insights into American political history could be gleaned by deeper analysis of party platforms.